...
function syncSidebar() { const value = averageProgress(); const label = document.querySelector('.sidebar-foot p strong'); const bar = document.querySelector('.mini-progress span'); if (label) label.textContent = `${value}%`; if (bar) bar.style.width = `${value}%`; }

function noteCard(note, compact = false) {
  return `<a class="note-card ${compact ? 'compact' : ''}" href="#/note/${note.slug}"><div class="card-top"><span class="topic-dot ${note.color}"></span><span>${note.topic}</span><time>${note.duration}</time></div><h3>${note.shortTitle}</h3><p>${note.summary}</p><div class="tag-row">${note.tags.slice(0,3).map(tag => `<span>${tag}</span>`).join('')}</div><div class="card-progress"><span>${note.progress === 100 ? '已完成' : note.progress ? `已读 ${note.progress}%` : '待读'}</span><i><b style="width:${note.progress}%"></b></i></div></a>`;
}

function renderHome() {
  const recent = [...notes].slice(0, 3);
  app.innerHTML = `<section class="overview-head"><div><p class="eyebrow">KNOWLEDGE BASE / 2026</p><h1>把零散输入，<br />编译成可复用的认知。</h1><p class="intro">围绕大模型后训练、Agent 与自进化，沉淀来自视频与论文的结构化学习笔记。</p></div><div class="signal-map" aria-label="知识图谱概览"><span class="node n1">Agent</span><span class="node n2">RL</span><span class="node n3">RSI</span><span class="node n4">Skill</span><svg viewBox="0 0 420 220" aria-hidden="true"><path d="M88 58 215 98 336 55M215 98l-92 84m92-84 112 77M123 182l214-7" /></svg><div class="map-stat"><strong>${allTags().length}</strong><span>概念节点</span></div></div></section>
  <section class="metric-row" aria-label="学习数据"><article><small>已整理笔记</small><strong>0${notes.length}</strong><span>来自视频学习</span></article><article><small>主题覆盖</small><strong>05</strong><span>Agent 为主线</span></article><article><small>学习完成度</small><strong>${averageProgress()}%</strong><span>${completedCount()} / ${notes.length} 已完成</span></article><article><small>累计素材</small><strong>6.1h</strong><span>视频与访谈</span></article></section>
...
document.getElementById('themeToggle')?.addEventListener('click', () => { const light = document.documentElement.classList.toggle('light'); localStorage.setItem('llm-fieldnotes-theme', light ? 'light' : 'dark'); });
if (localStorage.getItem('llm-fieldnotes-theme') === 'light') document.documentElement.classList.add('light');
